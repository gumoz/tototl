...
- (NSImage *)badgeOfSize:(float)size forValue:(unsigned)value overImage:(NSImage *)image insetX:(float)dx y:(float)dy andBadgeSize:(float)aBadgeSize
{
	NSImage *badgeOverlay = [self badgeOverlayImageForValue:value insetX:dx y:dy ofSize:size andBadgeSize:aBadgeSize];
...
- (NSImage *)badgeOverlayImageForValue:(unsigned)value insetX:(float)dx y:(float)dy ofSize:(float)size andBadgeSize:(float)aBadgeSize{

//	NSLog(@"\n---------------------------------------------------------- size: %f",size);
	NSImage *badgeImage = [self badgeOfSize:aBadgeSize forValue:value];
	
	//NSImage *badgeImage = [self largeBadgeForValue:value];
	
	
//	NSLog(@"badgeOverlay: %@", badgeImage);
	NSImage *overlayImage = [[NSImage alloc] initWithSize:NSMakeSize(size,size)];
	
	//draw large icon in the upper right corner of the overlay image
	[overlayImage lockFocus];
	NSSize badgeSize = [badgeImage size];
	
	float x = size - dx - badgeSize.width;
	float y = size - dy - badgeSize.height;
	NSPoint point = NSMakePoint(x,y);
	[badgeImage compositeToPoint:point operation:NSCompositeSourceOver];  
...
{
	NSImage *image = [self badgeOverlayImageForValue:value insetX:dx y:dy ofSize:128 andBadgeSize:CTLargeBadgeSize];
	return image;
}

- (void)badgeApplicationDockIconWithValue:(int)value insetX:(float)dx y:(float)dy
{
  NSImage *appIcon      = [NSImage imageNamed:@"NSApplicationIcon"];
  NSImage *badgeOverlay = [self badgeOverlayImageForValue:value insetX:dx y:dy];
