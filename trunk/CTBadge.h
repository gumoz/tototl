...
- (void)badgeApplicationDockIconWithValue:(int)value insetX:(float)dx y:(float)dy;		//Badges the Application's icon with <value>
																							//	and puts it on the dock

- (NSImage *)badgeOverlayImageForValue:(unsigned)value insetX:(float)dx y:(float)dy ofSize:(float)size andBadgeSize:(float)badgeSize;
